...
const CreateBorrowRequestValidation = z.object({
    ...
    borrowForDays: z.number({ required_error: "Borrow duration is required!" })
...
const ApproveBorrowRequestValidation = z.object({
    expireDate: z.date({ coerce: true, required_error: "Expire date is required!" })
        .refine((date) => {
            const now = new Date();
...
const RejectBorrowRequestValidation = z.object({
    rejectFor:z.string({required_error:"rejectedFor is required"}).nonempty("Rejected for can not be empty")
});
